...
public class TipoPersonaController : BaseController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<IEnumerable<object>>> Vigilantes()
    {
        var Vigilante = await _unitOfWork.TipoPersonas.Vigilantes();

        return Ok(Vigilante);
    }

    [HttpGet("EmpleadosSeguridad")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<IEnumerable<object>>> EmpleadosSeguridad()
    {
        var empleado = await _unitOfWork.TipoPersonas.EmpleadosSeguridad();
