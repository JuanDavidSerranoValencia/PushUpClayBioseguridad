...
public class PersonaController : BaseController
{
    ...
    [HttpGet("ContactosVigilantes")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<IEnumerable<object>>> ContactosVigilantes()
    {
        var contactos = await _unitOfWork.Personas.ContactosVigilantes();
    ...
    {
        var clientesBga = await _unitOfWork.Personas.ClientesBucaramanga();

        return Ok(clientesBga);
    }

    [HttpGet("GironPiedecuesta")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<IEnumerable<object>>> GironPiedecuesta()
    {
        var GironPiedecuesta = await _unitOfWork.Personas.GironPiedecuesta();

        return Ok(GironPiedecuesta);
    }
}
